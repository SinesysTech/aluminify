...
export class ProfessorAnalyticsService {
  ...
  async getProfessorDashboard(
    ...
    const professorNome = (professor as { users: { full_name: string | null } })?.users?.full_name ?? 'Professor'

    // Buscar métricas em paralelo
    const [summary, alunos, agendamentos, performanceAlunos] = await Promise.all([
  ...
  async getStudentsUnderCare(
    ...
    for (const [alunoId, alunoInfo] of alunosMap) {
      ...
      result.push({
        ...
        cursoNome: (alunoCurso?.cursos as { nome: string })?.nome ?? 'Sem curso',
        progresso,
  ...
  async getUpcomingAppointments(
    ...
    return agendamentos.map((agendamento) => {
      ...
      return {
        ...
        status: agendamento.status as 'pendente' | 'confirmado' | 'cancelado' | 'realizado',
  ...
  private async getPerformanceAlunos(
    ...
    for (const disciplina of disciplinas) {
      ...
      const corretas = respostas.filter((r: { correta: boolean }) => r.correta).length
      const aproveitamentoMedio = Math.round((corretas / respostas.length) * 100)
