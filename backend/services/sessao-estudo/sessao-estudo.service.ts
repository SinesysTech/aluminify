...
function parseLogPausas(logs: LogPausa[]): LogPausa[] {
  ...
  return logs.map((log, index) => {
    ...
    if (!log.inicio || !log.fim) {
      throw new SessaoEstudoValidationError(
        `log_pausas[${index}] precisa de inicio e fim`,
      );
    }
    if (!isISODate(log.inicio) || !isISODate(log.fim)) {
      throw new SessaoEstudoValidationError(
        `log_pausas[${index}] deve usar datas ISO`,
      );
    }
    if (log.tipo !== "manual" && log.tipo !== "distracao") {
      throw new SessaoEstudoValidationError(
        `log_pausas[${index}].tipo inválido`,
      );
    }
...
function calcularTempos(
  ...
  if (!isISODate(inicioIso) || !isISODate(fimIso)) {
    throw new SessaoEstudoValidationError(
      "Datas de início e fim devem estar em ISO válido",
    );
  }
  ...
  for (const pausa of logPausas) {
    ...
    if (
      Number.isNaN(pausaInicio) ||
      Number.isNaN(pausaFim) ||
      pausaFim <= pausaInicio
    ) {
...
export class SessaoEstudoService {
  ...
  async finalizarSessao(
    ...
    const { tempoTotalBrutoSegundos, tempoTotalLiquidoSegundos } =
      calcularTempos(sessao.inicio, fimIso, logPausas);

    const result = await this.repository.updateFinalizacao(
      input.sessaoId,
      alunoId,
      {
        fimIso,
  ...
  async heartbeat(alunoId: string, sessaoId: string): Promise<void> {
    ...
    if (cachedState && now - cachedState.lastHeartbeat < 5 * 60 * 1000) {
      await cacheService.set(
        cacheKey,
        { lastHeartbeat: now, needsUpdate: true },
        600,
      ); // TTL: 10 minutos
    ...
    await cacheService.set(
      cacheKey,
      { lastHeartbeat: now, needsUpdate: false },
      600,
    ); // TTL: 10 minutos
  }
}

export const sessaoEstudoService = new SessaoEstudoService(
  new SessaoEstudoRepository(),
);
