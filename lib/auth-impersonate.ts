 ...
 */
export async function setImpersonationContext(context: ImpersonationContext): Promise<void> {
...
export function canImpersonateUser(
  ...
  // Professores e admins de empresa podem impersonar apenas alunos
  if (realUserRole === 'professor' || realUserRole === 'empresa') {
  ...
  return { allowed: false, reason: 'Você não tem permissão para impersonar usuários' }
}
