...
export class DatabasePatternAnalyzer extends BasePatternAnalyzer {
   ...
   */
  private detectDatabaseClientPatterns(
    file: FileInfo,
    ast: SourceFile,
  ): Issue[] {
    const issues: Issue[] = [];
    ...
    for (const call of callExpressions) {
      ...
      for (const pattern of dbClientPatterns) {
        if (callText.includes(pattern)) {
          ...
          const uniquePatterns = new Set(
            this.dbClientPatterns.map((p) => p.functionName),
          );
          if (uniquePatterns.size > 1) {
            issues.push(
              this.createIssue({
                ...
                recommendation:
                  "Standardize database client creation to use a single pattern across the codebase. For Supabase projects, use the appropriate client creation method based on context (server component, route handler, middleware, etc.) but maintain consistency within each context type.",
      ...
      if (
        callText.includes("supabase.from") ||
    ...
    for (const varDecl of variableDeclarations) {
      const _varText = varDecl.getText();
      ...
      if (
        ...
        if (Node.isVariableDeclaration(varDecl)) {
          const initializerText =
            varDecl.getInitializer()?.getText().toLowerCase() || "";
          if (
            dbClientPatterns.some((pattern) =>
      ...
      if (Node.isVariableDeclaration(parent)) {
        ...
        if (varText.includes("error") && varText.includes("{")) {
          ...
          if (block) {
            ...
            const afterDeclaration = blockText.substring(
              blockText.indexOf(varText) + varText.length,
            );
  ...
  private detectInconsistentErrorHandling(
        ...
        if (!hasErrorHandling) {
          issues.push(
            this.createIssue({
              ...
              recommendation:
                "Add proper error handling using try-catch blocks or by checking the error property returned from Supabase operations. Example: const { data, error } = await supabase.from(...); if (error) { /* handle error */ }",
    ...
    if (dbOperations.length > 1) {
      const withErrorHandling = dbOperations.filter(
        (op) => op.hasErrorHandling,
      ).length;
      const withoutErrorHandling = dbOperations.filter(
        (op) => !op.hasErrorHandling,
      ).length;
      ...
      if (withErrorHandling > 0 && withoutErrorHandling > 0) {
        const percentage = Math.round(
          (withoutErrorHandling / dbOperations.length) * 100,
        );
        ...
        issues.push(
          this.createIssue({
            ...
            recommendation:
              "Standardize error handling across all database operations. Either use try-catch blocks consistently or always check the error property from Supabase responses.",
            estimatedEffort: "medium",
   ...
   */
  private detectInconsistentTypeUsage(
    file: FileInfo,
    ast: SourceFile,
  ): Issue[] {
    const issues: Issue[] = [];
    ...
    const typeAnnotations: Map<
      string,
      Array<{ type: string; node: Node }>
    > = new Map();
    ...
    for (const varDecl of variableDeclarations) {
      ...
      if (varName && typeNode) {
        ...
        const isDatabaseType =
          ...
          varName.match(
            /^(user|profile|post|comment|session|account|organization|team|project|task|item|entity|record|data)s?$/i,
          );
        ...
        if (this.extractDatabaseOperation(initializerText)) {
          issues.push(
            this.createIssue({
              ...
              recommendation:
                "Use proper TypeScript types for database entities. For Supabase, use generated types from the Database type or define explicit interfaces for your entities.",
              estimatedEffort: "small",
        ...
        if (uniqueTypes.size > 1) {
          issues.push(
            this.createIssue({
              ...
              recommendation:
                "Standardize type definitions for database entities. Use Supabase generated types consistently or define a single canonical type for each entity in a shared types file.",
              estimatedEffort: "medium",
    ...
    for (const typeAlias of typeAliases) {
      ...
      if (
        typeName.match(
          /^(User|Profile|Post|Comment|Session|Account|Organization|Team|Project|Task|Item|Entity|Record)s?$/,
        )
      ) {
        // Check if there's a Supabase Database type import
        const imports = ast.getImportDeclarations();
        const hasSupabaseTypes = imports.some(
          (imp) =>
        ...
        if (hasSupabaseTypes && !typeText.includes("Database[")) {
          issues.push(
            this.createIssue({
              ...
              recommendation:
                "Consider using Supabase generated types instead of manual definitions. If manual types are necessary, ensure they match the database schema exactly.",
              estimatedEffort: "small",
        ...
        if (hasSupabaseTypes && !interfaceText.includes("Database[")) {
          issues.push(
            this.createIssue({
              ...
              recommendation:
                "Consider using Supabase generated types instead of manual definitions. If manual interfaces are necessary, ensure they match the database schema exactly.",
              estimatedEffort: "small",
              tags: ["database", "type-safety", "supabase"],
            }),
          );
        ...
        if (operation) {
          issues.push(
            this.createIssue({
              ...
              recommendation:
                "Move database operations to a service layer or API route. Components should fetch data through API calls or use server-side data fetching patterns (e.g., Next.js Server Components, getServerSideProps).",
              estimatedEffort: "medium",
          ...
          if (argText.includes("${") || argText.includes("+")) {
            issues.push(
              this.createIssue({
                ...
                recommendation:
                  "Use parameterized queries or Supabase query builder methods instead of string concatenation. Pass user input as separate parameters that will be properly escaped.",
                estimatedEffort: "small",
      ...
      if (dbOperationCount > 3) {
        issues.push(
          this.createIssue({
            ...
            recommendation:
              "Extract database operations into a dedicated service module. This improves code organization, makes testing easier, and allows reuse across multiple API routes.",
            estimatedEffort: "medium",
    ...
    if (clientImports.length > 2) {
      issues.push(
        this.createIssue({
          ...
          recommendation:
            "Standardize on a single client creation pattern appropriate for the context. For Next.js with Supabase, use createServerComponentClient for Server Components, createRouteHandlerClient for API routes, etc.",
          estimatedEffort: "small",
  ...
  private detectUnnecessaryAdapters(file: FileInfo, ast: SourceFile): Issue[] {
    ...
    for (const func of functions) {
      ...
      if (isPassThrough) {
        const recommendation = this.generateAdapterRecommendation(
          func,
          funcName,
          bodyText,
        );
  ...
  private isPassThroughWrapper(
    ...
    if (
      ...
      bodyText.includes(".transform(") ||
      bodyText.includes("Object.assign") ||
      bodyText.includes("...")
    ) {
      ...
      if (hasReturnStatement) {
        const returnStmt = statements.find((stmt) =>
          Node.isReturnStatement(stmt),
        );
        if (returnStmt) {
          ...
          const isSimpleReturn = /^return\s+\w+\([^)]*\)/.test(
            returnText.trim(),
          );
          if (!isSimpleReturn) {
            return false;
    ...
    const isSingleExpression =
      statements.length === 1 && Node.isExpressionStatement(statements[0]);
    ...
    const passThrough =
      paramNames.length > 0 && paramsUsedInCall >= paramNames.length * 0.7; // At least 70% of params passed through

    return (
      passThrough || (statements.length === 1 && callExpressions.length === 1)
    );
  }
   ...
   */
  private generateAdapterRecommendation(
    func: Node,
    funcName: string,
    bodyText: string,
  ): string {
    ...
    return (
      `Remove the wrapper function '${funcName}' and call ${calledFunction} directly. ` +
