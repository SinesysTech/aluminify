...
class MockAnalyzer {
    ...
    supportedTypes;
    issueGenerator;
    constructor(supportedTypes, issueGenerator) {
        this.supportedTypes = supportedTypes;
        this.issueGenerator = issueGenerator;
    }
    async analyze(file, ast) {
...
function createMockFile(relativePath, category = 'util', content = 'const x = 1;') {
    return {
...
function createMockIssue(file, overrides = {}) {
    return {
...
describe('AnalysisEngine', () => {
    ...
    describe('analyze', () => {
        ...
        it('should stop on error when continueOnError is false', async () => {
            ...
            const analyzer = new MockAnalyzer(['util'], async () => []);
            try {
                await expect(engineStopOnError.analyze(files, [analyzer])).rejects.toThrow(AnalysisError);
            }
            finally {
                await fs.rm(tmpDir, { recursive: true, force: true });
            }
    ...
    describe('error tracking', () => {
        ...
        it('should stop after maxErrors', async () => {
            ...
            const analyzer = new MockAnalyzer(['util'], async () => []);
            await expect(engineWithMaxErrors.analyze(files, [analyzer])).rejects.toThrow(AnalysisError);
        });
    });
    describe('reset', () => {
