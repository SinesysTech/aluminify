...
describe('ServicePatternAnalyzer', () => {
    ...
    describe('Basic Functionality', () => {
        ...
        it('should discover service modules', async () => {
            const sourceFile = project.createSourceFile('backend/services/user/index.ts', `
...
export function createUser(data: any) {
  return { id: '123', ...data };
        ...
        it('should track service dependencies', async () => {
            ...
            const serviceA = project.createSourceFile('backend/services/serviceA/index.ts', `
...
export function functionA() {
  return functionB();
        ...
        `.trim());
            const serviceB = project.createSourceFile('backend/services/serviceB/index.ts', `
...
export function functionB() {
  return functionA();
        ...
        `.trim());
            ...
            const circularDepIssues = allIssues.filter(issue => issue.tags.includes('circular-dependency'));
...
export function functionB() {
  return functionC();
        ...
        `.trim());
            ...
            const serviceC = project.createSourceFile('backend/services/serviceC/index.ts', `
import { functionA } from '../serviceA';
  ...
  return 'B';
    ...
    describe('Issue Detection', () => {
        it('should detect service with no exports', async () => {
            const sourceFile = project.createSourceFile('backend/services/empty/index.ts', `
// Empty service file
const internal = 'value';
        `.trim());
            ...
            const noExportsIssue = issues.find(issue => issue.description.includes('has no exports'));
        ...
        it('should detect excessive service dependencies', async () => {
            const sourceFile = project.createSourceFile('backend/services/complex/index.ts', `
...
export function complexFunction() {
  return a() + b() + c() + d() + e() + f();
        ...
        `.trim());
            ...
            const issues = await analyzer.analyze(fileInfo, sourceFile);
            const excessiveDepsIssue = issues.find(issue => issue.description.includes('depends on') && issue.description.includes('services'));
            expect(excessiveDepsIssue).toBeDefined();
            expect(excessiveDepsIssue?.severity).toBe('medium');
        });
    });
});
//# sourceMappingURL=service-pattern-analyzer.test.js.map