...
describe('DatabasePatternAnalyzer', () => {
    ...
    describe('Task 6.1: Database Client Pattern Detection', () => {
      ...
      `;
            ...
            const inconsistentPatterns = issues.filter(issue => issue.type === 'inconsistent-pattern' && issue.category === 'database');
      ...
      `;
            ...
            const issues = await analyzer.analyze(fileInfo, ast);
            const inconsistentIssue = issues.find(issue => issue.type === 'inconsistent-pattern');
      ...
      `;
        ...
        it('should track multiple patterns across analysis', async () => {
            ...
            const inconsistentIssues = issues2.filter(issue => issue.type === 'inconsistent-pattern');
            expect(inconsistentIssues.length).toBeGreaterThan(0);
        });
