...
describe('Property 1: Complete File Discovery', () => {
   ...
   */
  const simpleStructureArb: fc.Arbitrary<DirectoryStructure> = fc.dictionary(
    fileNameArb,
    fc.constant('// test file content')
  );
  ...
  const nestedStructureArb = fc.letrec(tie => ({
    structure: fc.oneof(
      ...
      fc.dictionary(
        ...
        fc.oneof(
          fc.constant('// test file content'),
          tie('structure') as fc.Arbitrary<DirectoryStructure>
        ),
        { minKeys: 1, maxKeys: 3 }
      )
    )
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (structure) => {
          ...
          const expectedCount = countExpectedFiles(
            structure,
  ...
  it('should respect include patterns and discover only matching files', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        async (structure, includePatterns) => {
          const testDir = await createTestDir();
          await createDirectoryStructure(testDir, structure);
          ...
          files.forEach(file => {
            const matches = includePatterns.some(pattern =>
              matchPattern(file.relativePath, pattern)
            );
