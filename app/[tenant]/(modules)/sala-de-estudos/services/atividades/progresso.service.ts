...
import { ProgressoAtividadeRepository } from "./progresso.repository";
import {
  ProgressoNotFoundError,
  ProgressoValidationError,
} from "./progresso.errors";
...
export class ProgressoAtividadeService {
  ...
  async updateStatus(
    ...
    const progresso = await this.repository.findOrCreateProgresso(
      alunoId,
      atividadeId,
      status,
      empresaId ?? null,
  ...
  async marcarComoConcluidoComDesempenho(
    ...
    if (desempenho.questoesTotais < 1) {
      throw new ProgressoValidationError(
        "Questões totais deve ser pelo menos 1",
      );
    }

    if (desempenho.questoesAcertos < 0) {
      throw new ProgressoValidationError(
        "Questões acertadas não pode ser negativo",
      );
    }

    if (desempenho.questoesAcertos > desempenho.questoesTotais) {
      throw new ProgressoValidationError(
        "Questões acertadas não pode ser maior que questões totais",
      );
    }
  ...
  async updateProgresso(
    ...
    if (payload.dataConclusao && payload.dataInicio) {
      const inicio =
        payload.dataInicio instanceof Date
          ? payload.dataInicio
          : new Date(progresso.dataInicio || Date.now());
      const conclusao =
        payload.dataConclusao instanceof Date
          ? payload.dataConclusao
          : new Date(payload.dataConclusao);
      if (conclusao < inicio) {
        throw new ProgressoValidationError(
  ...
  private async ensureExists(id: string): Promise<ProgressoAtividade> {
    ...
    if (!progresso) {
      throw new ProgressoNotFoundError(
        `Progresso with id "${id}" was not found`,
      );
    }
