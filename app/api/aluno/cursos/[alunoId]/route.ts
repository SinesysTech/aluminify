...
import { requireAuth, type AuthenticatedRequest } from '@/backend/auth/middleware'
  ...
  try {
    ...
    if (request.user && request.user.role !== 'professor' && request.user.role !== 'superadmin') {
      if (request.user.id !== alunoId) {
        return NextResponse.json({ error: 'Forbidden: You can only access your own courses' }, { status: 403 })
      }
    }
    ...
    const cursos = (data || [])
      .map((row: { cursos: { id: string; nome: string } | { id: string; nome: string }[] | null }) => {
        const c = row.cursos
        if (!c) return null
    ...
    // Deduplicar por id
    const unique = Array.from(new Map(cursos.map((c) => [c.id, c])).values()).sort((a, b) => a.nome.localeCompare(b.nome))
