...
async function getHandler(
  ...
  try {
    ...
    console.log('[Estatísticas Semanas API] Calculando estatísticas para cronograma:', cronogramaId);
    const result = await cronogramaService.calcularEstatisticasPorSemana(
      cronogramaId,
      request.user.id,
    );
    console.log('[Estatísticas Semanas API] Estatísticas calculadas com sucesso');

    return NextResponse.json(result, { status: 200 });
  } catch (error) {
    console.error('[Estatísticas Semanas GET] Error:', error);
    console.error('[Estatísticas Semanas GET] Error type:', error?.constructor?.name);
    console.error('[Estatísticas Semanas GET] Error message:', error instanceof Error ? error.message : String(error));
    console.error('[Estatísticas Semanas GET] Error stack:', error instanceof Error ? error.stack : 'N/A');
