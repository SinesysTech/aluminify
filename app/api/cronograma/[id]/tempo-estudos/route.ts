...
async function putHandler(
  ...
  try {
    ...
    if (!cronogramaId) {
      return NextResponse.json({ error: 'ID do cronograma é obrigatório' }, { status: 400 });
    }
    ...
    if (!data || !disciplina_id || !frente_id || typeof tempo_estudos_concluido !== 'boolean') {
      return NextResponse.json(
        { error: 'Campos obrigatórios: data, disciplina_id, frente_id, tempo_estudos_concluido' },
        { status: 400 },
      );
    }
    ...
    if (existente) {
      ...
      if (updateError) {
        ...
        throw new Error(`Erro ao atualizar tempo de estudos: ${updateError.message}`);
      }
      ...
      if (createError) {
        ...
        throw new Error(`Erro ao criar registro de tempo de estudos: ${createError.message}`);
      }

      resultado = created;
    }

    return NextResponse.json({ success: true, tempo_estudos: resultado }, { status: 200 });
  } catch (error) {
...
async function getHandler(
  ...
  try {
    ...
    if (selectError) {
      ...
      throw new Error(`Erro ao buscar tempo de estudos: ${selectError.message}`);
    }

    return NextResponse.json({ success: true, tempo_estudos: tempoEstudos || [] }, { status: 200 });
  } catch (error) {
