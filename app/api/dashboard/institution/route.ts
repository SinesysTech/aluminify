...
async function getHandler(request: AuthenticatedRequest) {
  try {
    ...
    const isEmpresaAdmin = request.user?.isEmpresaAdmin
    ...
    if (!isEmpresaAdmin && request.user?.role !== 'superadmin') {
      return NextResponse.json(
        { error: 'Acesso negado. Apenas administradores da instituição podem acessar este dashboard.' },
        { status: 403 }
      )
    }
    ...
    const dashboardData = await institutionAnalyticsService.getInstitutionDashboard(
      empresaId,
      period
    )
