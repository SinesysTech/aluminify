...
async function getHandler(request: AuthenticatedRequest) {
  try {
    ...
    const data = await dashboardAnalyticsService.getCoursesWithProgress(targetUserId, empresaId)
    return NextResponse.json({ data })
  } catch (error) {
    console.error("Dashboard Courses List API Error:", error)
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Erro ao carregar lista de cursos" },
      { status: 500 },
    )
  }
}
