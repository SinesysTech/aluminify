...
async function getHandler(request: AuthenticatedRequest) {
  try {
    ...
    const data = await dashboardAnalyticsService.getLearningPaths(targetUserId)
    return NextResponse.json({ data })
  } catch (error) {
    console.error("Dashboard Learning Paths API Error:", error)
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Erro ao carregar trilhas de aprendizado" },
      { status: 500 },
    )
  }
}
