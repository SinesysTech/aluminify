...
import { atividadeService, atividadeRequerDesempenho } from '@/backend/services/atividade';
...
async function patchHandler(request: AuthenticatedRequest, params: { atividadeId: string }) {
  try {
    ...
    if (!status) {
      return NextResponse.json({ error: 'Missing parameter: status is required' }, { status: 400 });
    }
    ...
    if (status === 'Concluido' && body.desempenho) {
      ...
      if (atividadeRequerDesempenho(atividade.tipo)) {
        return NextResponse.json(
          { error: 'Este tipo de atividade requer registro de desempenho. Forneça os dados no campo "desempenho".' },
          { status: 400 }
        );
      }
    }
    ...
    const updated = await progressoAtividadeService.updateStatus(alunoId, params.atividadeId, status);
    return NextResponse.json({ data: serializeProgresso(updated) });
  } catch (error) {
