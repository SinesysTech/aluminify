...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: frentesData, error: frentesError } = await supabaseAdmin.from("frentes").select("id").in("disciplina_id", input.disciplinas_ids);
    if (frentesError) {
    ...
    const { data: modulosData, error: modulosError } = await supabaseAdmin.from("modulos").select("id").in("frente_id", frenteIds);
    if (modulosError) {
    ...
    const { data: aulasData, error: aulasError } = await supabaseAdmin.from("aulas").select(`
        id,
        nome,
        numero_aula,
      ...
      `).in("modulo_id", moduloIds).gte("prioridade", input.prioridade_minima);
    if (aulasError) {
